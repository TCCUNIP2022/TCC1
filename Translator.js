function Translator() {
  ...
  this.speakTextUsingGoogleSpeaker = function (args) {
    ...
    var audio_url =
      "https://translate.google.com/translate_tts?ie=UTF-8&total=1&idx=0&textlen=" +
      textToSpeak.length +
      "&tl=" +
      targetLanguage +
      "&q=" +
  ...
  function initTranscript(callback, language) {
    ...
    recognition.onerror = function (e) {
      if (e.error === "audio-capture") {
        ...
        alert(
          "Failed capturing audio i.e. microphone. Please check console-logs for hints to fix this issue."
        );
        console.error(
          "No microphone was found. Ensure that a microphone is installed and that microphone settings are configured correctly. https://support.google.com/chrome/bin/answer.py?hl=en&answer=1407892"
        );
  ...
  self.processInWebWorker = function (args) {
    ...
    if (!self.speakWorker && args.onWorkerFileDownloadStart)
      args.onWorkerFileDownloadStart();

    var blob = URL.createObjectURL(
      new Blob(
        [
          'importScripts("' +
            (args.workerPath || "Robot-Speaker.js") +
            '");this.onmessage =  function (event) {postMessage(generateSpeech(event.data.text, event.data.args));}; postMessage("worker-file-downloaded");',
        ],
        {
  ...
  var Speaker = {
    Speak: function (text, args) {
      ...
      if (_args.onWorkerFileDownloadStart)
        delete _args.onWorkerFileDownloadStart;

      speakWorker.postMessage({ text: text, args: _args });
    },
  };

  var Google_Translate_API_KEY = "AIzaSyAqMUjk7jRtMFJYdJZw3BgIwkjHUqpgr0s";
}
